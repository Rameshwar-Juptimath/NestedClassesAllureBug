...
public class AttributeGroupsPageObjects extends PageFactoryInitializer {
	...
	public AttributeGroupsPageObjects enterAttributeGroupField(String attributeGroupName) throws InterruptedException {
		...
		Assert.assertTrue(attributeGroupNameTextBoxLocator.isDisplayed(),
				"Attribute Group Name textbox is not displayed.");
		Thread.sleep(2000);
	...
	@Step("This method is used verify the Attribute Group field")
	public AttributeGroupsPageObjects enterEditAttributeGroupField(String editAttributeGroupName)
	...
	public AttributeGroupsPageObjects enterTheUpdateAttributeGroupNameInSearchField(String updateAttributeGroupName)
			throws InterruptedException {
	...
	public AttributeGroupsPageObjects enterAttributeGroupDescriptionField(String attributeGroupDescription)
			throws InterruptedException {
		Thread.sleep(2000);
	...
	public AttributeGroupsPageObjects verifyAttributeGroupSavedSuccessfulMessage(
			String attributeGroupSavedSuccessfulMessage) {
		waiting.explicitWaitVisibilityOfElement(attributeGroupSavedSuccessfulMessageLocator, 6);
		Assert.assertEquals(attributeGroupSavedSuccessfulMessageLocator.getText().trim(),
				attributeGroupSavedSuccessfulMessage.trim(),
				"Invalid  message. Getting " + attributeGroupSavedSuccessfulMessageLocator.getText().trim() + ".");
		return this;
	}
	...
	public AttributeGroupsPageObjects verifyNoResultsFoundMessage(String noResultsFoundErrorMessage) {
		...
		Assert.assertEquals(noResultsFoundErrorMessageLocator.getText().trim(), noResultsFoundErrorMessage.trim(),
				"Invalid  message. Getting " + noResultsFoundErrorMessageLocator.getText().trim() + ".");
		return this;
	}

	@Step("This method is used verify Attribute GroupName Required Message ")
	public AttributeGroupsPageObjects verifyAttributeGroupNameRequiredMessage(
			String attributeGroupNameRequiredMessage) {
		waiting.explicitWaitVisibilityOfElement(attributeGroupNameRequiredMessageLocator, 30);
		Assert.assertEquals(attributeGroupNameRequiredMessageLocator.getText().trim(),
				attributeGroupNameRequiredMessage.trim(),
				"Invalid  message. Getting " + attributeGroupNameRequiredMessageLocator.getText().trim() + ".");
		return this;
	}

	@Step("This method is used verify FieldNames In Add New AttributeGroup")
	public AttributeGroupsPageObjects verifyFieldNamesInAddNewAttributeGroup(String fieldsNamesInAddNewAttributeGroup)
			throws Exception {
		...
		for (int i = 0; i < fieldNamesInAddNewAttributeGroupLocator.size(); i++) {
			Assert.assertEquals(fieldNamesInAddNewAttributeGroupLocator.get(i).getText().trim(),
					fieldNamesInAddNewAttributeGroup[i].trim());
		}
	...
	public AttributeGroupsPageObjects verifyAttributeGroupUpdateSavedSuccessfulMessage(
			String attributeGroupUpdateSavedSuccessfulMessage) {
		waiting.explicitWaitVisibilityOfElement(attributeUpdateSavedSuccessfulMessageLocator, 6);
		Assert.assertTrue(
				attributeUpdateSavedSuccessfulMessageLocator.getText().trim()
						.equalsIgnoreCase(attributeGroupUpdateSavedSuccessfulMessage),
				"Invalid  message. Getting " + attributeUpdateSavedSuccessfulMessageLocator.getText().trim() + ".");
		return this;
	}

	@Step("This method is used to remove the Attribute ")
	public AttributeGroupsPageObjects clickOnRemoveUpdateAttributeGroup(String updateAttributeGroupName)
			throws InterruptedException {
		Thread.sleep(3000);
		WebElement wb = getDriver().findElement(By.xpath("(//td[contains(text(),'" + updateAttributeGroupName
	...
	@Step("This method is used to remove the Attribute ")
	public AttributeGroupsPageObjects clickOnRemoveAttributeGroup(String attributeGroupName)
			throws InterruptedException {
		Thread.sleep(3000);
		WebElement wb = getDriver().findElement(By.xpath("(//td[contains(text(),'" + attributeGroupName
				+ "')]/preceding-sibling::td[@class='rich-table-cell']/div/input)[2]"));
		wb.click();
		return this;
	}
	...
	public AttributeGroupsPageObjects verifySuccessfulMessageAfterDeletionAttributeGroup(
			String attributeGroupRemovedSuccessfulMessage) {

		waiting.explicitWaitVisibilityOfElement(deletedSuccessfulMessageOfAttributeGroupLocator, 15);
		Assert.assertEquals(
				deletedSuccessfulMessageOfAttributeGroupLocator.getText().trim()
						.equals(attributeGroupRemovedSuccessfulMessage.trim()),
				"Invalid  message. Getting " + deletedSuccessfulMessageOfAttributeGroupLocator.getText().trim() + ".");
		return this;
	}
	...
	public AttributeGroupsPageObjects verifyUpdateAttributeGroupDeletion(String updateAttributeGroupName) {
		Assert.assertTrue(assertUpdateAttributeGroupNameDeletion(updateAttributeGroupName),
				"Group name is not deleted");
		return this;
	}
	...
	public AttributeGroupsPageObjects verifyTextboxValidation(String fieldName, String textToBeEnterInTheTextbox,
		...
		case "Attribute GroupName":
			...
			Assert.assertEquals(attributeGroupNameTextBoxLocator.getAttribute("value").trim(),
					textToBeEnterInTheTextbox.substring(0, Math.min(textToBeEnterInTheTextbox.length(),
		...
		case "Attribute GroupDes":
			...
			Assert.assertEquals(attributeGroupDesTextBoxLocator.getAttribute("value").trim(),
					textToBeEnterInTheTextbox.substring(0, Math.min(textToBeEnterInTheTextbox.length(),
							maximumNumberOfCharactersAcceptedByTheTexbox)));
			break;
		}
