...
public class AddNewItemPageObjects extends PageFactoryInitializer {
	...
	@FindAll(value = {
			@FindBy(xpath = "//div[@class='tabContainerLabel' and not(contains(descendant::span,'*')) and text()]") })
	...
	public AddNewItemPageObjects verifyEnableTabsOfItemEdit() {
		...
		Assert.assertTrue(descriptionsTabLocator.getAttribute("class").contains("active"),
				"descriptions tab is  enabled");
		...
		Assert.assertTrue(documentsTabLocator.getAttribute("class").contains("active"),
				"Documents info tab is enabled");
		...
		Assert.assertTrue(linkedItemsTabLocator.getAttribute("class").contains("active"),
				"Linked Items tab is  enabled");
		Assert.assertTrue(attributesTabLocator.getAttribute("class").contains("active"), "Attributes tab is enabled");
		Assert.assertTrue(customPricesTabLocator.getAttribute("class").contains("active"),
				"Custom prices tab is enabled");
		Assert.assertTrue(categorizationTabLocator.getAttribute("class").contains("active"),
				"categorization tab is enabled");
		Assert.assertTrue(customFieldsTabLocator.getAttribute("class").contains("active"),
				"Custom fields tab is enabled");
		Assert.assertTrue(warehouseTabLocator.getAttribute("class").contains("active"), "Warehouse tab is enabled");
		Assert.assertTrue(customerPartNumberTabLocator.getAttribute("class").contains("active"),
				"Customer Part number tab is enabled");
		Assert.assertTrue(blogLinkTabLocator.getAttribute("class").contains("active"), "Blog Link info tab is enabled");
		Assert.assertTrue(userReviewTabLocator.getAttribute("class").contains("active"),
				"User review info tab is enabled");
		return this;
	}

	public AddNewItemPageObjects enterManufactrerPartNumber(String getmanufacturerPartNumber)
	...
	public AddNewItemPageObjects verifyTextboxValidation(String fieldName, String textToBeEnterInTheTextbox,
			int maximumNumberOfCharactersAcceptedByTheTexbox) throws Exception {
		...
		case "Part Number":
			...
			Assert.assertEquals(partNumberTextBoxLocator.getAttribute("value").trim(),
					textToBeEnterInTheTextbox.substring(0,
							Math.min(textToBeEnterInTheTextbox.length(), maximumNumberOfCharactersAcceptedByTheTexbox)),
					"PartNumber input Field is allowing more than acceptance value.");
			break;
		...
		case "Manufacturer Part Number":
			...
			Assert.assertEquals(manufacturerPartNumberTextBoxLocator.getAttribute("value").trim(),
					textToBeEnterInTheTextbox.substring(0,
							Math.min(textToBeEnterInTheTextbox.length(), maximumNumberOfCharactersAcceptedByTheTexbox)),
					" Manufacturer PartNumber input Field is allowing more than acceptance value.");
			break;
		...
		case "Competitor Part Number":
			...
			Assert.assertEquals(competitorPartNumberTextBoxLocator.getAttribute("value").trim(),
					textToBeEnterInTheTextbox.substring(0,
							Math.min(textToBeEnterInTheTextbox.length(), maximumNumberOfCharactersAcceptedByTheTexbox)),
					"CompetetorPartNumber input Field is allowing more than acceptance value.");
			break;
		...
		case "UPC":
			...
			Assert.assertEquals(upcTextBoxLocator.getAttribute("value").trim(),
					textToBeEnterInTheTextbox.substring(0,
							Math.min(textToBeEnterInTheTextbox.length(), maximumNumberOfCharactersAcceptedByTheTexbox)),
					"UPC input Field is allowing more than acceptance value.");
			break;
		...
		case "Data Source":
			...
			Assert.assertEquals(dataSourceTextBoxLocator.getAttribute("value").trim(),
					textToBeEnterInTheTextbox.substring(0,
							Math.min(textToBeEnterInTheTextbox.length(), maximumNumberOfCharactersAcceptedByTheTexbox)),
					"Data Source input Field is allowing more than acceptance value.");
			break;
		...
		case "Length":
			...
			Assert.assertEquals(lengthTextBoxLocator.getAttribute("value").trim(),
					textToBeEnterInTheTextbox.substring(0,
							Math.min(textToBeEnterInTheTextbox.length(), maximumNumberOfCharactersAcceptedByTheTexbox)),
					"Length input Field is allowing more than acceptance value.");
			break;
		...
		case "Width":
			...
			Assert.assertEquals(widthTextBoxLocator.getAttribute("value").trim(),
					textToBeEnterInTheTextbox.substring(0,
							Math.min(textToBeEnterInTheTextbox.length(), maximumNumberOfCharactersAcceptedByTheTexbox)),
					"Width input Field is allowing more than acceptance value.");
			break;
		...
		case "Height":
			...
			Assert.assertEquals(heightTextBoxLocator.getAttribute("value").trim(),
					textToBeEnterInTheTextbox.substring(0,
							Math.min(textToBeEnterInTheTextbox.length(), maximumNumberOfCharactersAcceptedByTheTexbox)),
					"Hieght input Field is allowing more than acceptance value.");
			break;
		...
		case "Weight":
			...
			Assert.assertEquals(weightTextBoxLocator.getAttribute("value").trim(),
					textToBeEnterInTheTextbox.substring(0,
							Math.min(textToBeEnterInTheTextbox.length(), maximumNumberOfCharactersAcceptedByTheTexbox)),
					"Weight input Field is allowing more than acceptance value.");
			break;
		...
		case "Quantity Available":
			...
			Assert.assertEquals(quantityAvailableTextBoxLocator.getAttribute("value").trim(),
					textToBeEnterInTheTextbox.substring(0,
							Math.min(textToBeEnterInTheTextbox.length(), maximumNumberOfCharactersAcceptedByTheTexbox)),
					"qty availability input Field is allowing more than acceptance value.");
			break;
		...
		case "List Price":
			...
			Assert.assertEquals(listPriceTextBoxLocator.getAttribute("value").trim(),
					textToBeEnterInTheTextbox.substring(0,
							Math.min(textToBeEnterInTheTextbox.length(), maximumNumberOfCharactersAcceptedByTheTexbox)),
					"List price input Field is allowing more than acceptance value.");
			break;
		...
		case "Cost Price":
			...
			Assert.assertEquals(costPriceTextBoxLocator.getAttribute("value").trim(),
					textToBeEnterInTheTextbox.substring(0,
							Math.min(textToBeEnterInTheTextbox.length(), maximumNumberOfCharactersAcceptedByTheTexbox)),
					"Cost Price input Field is allowing more than acceptance value.");
			break;
	...
	public AddNewItemPageObjects verifyAttributtesInTheRightNavigationBar(
			String[] attributesInRightNavigationBarOfAddNewItemsPage) throws Exception {
		Thread.sleep(2500);
		for (int i = 0; i < attributesInTheRightNavigationBarLocator.size(); i++) {
			Assert.assertEquals(attributesInTheRightNavigationBarLocator.get(i).getText().trim(),
					attributesInRightNavigationBarOfAddNewItemsPage[i]);
		}
	...
	public AddNewItemPageObjects verifyEnableAndDisableOfTabs() {
		...
		waiting.explicitWaitVisibilityOfElement(generalInfoTabLocator, 10);
		Assert.assertTrue(generalInfoTabLocator.getAttribute("class").contains("active"),
				"General info tab is not enabled");
		Assert.assertFalse(generalInfoTabLocator.getAttribute("class").contains("disabled"),
				"General info tab is not enabled");
		Assert.assertTrue(descriptionsTabLocator.getAttribute("class").contains("disabled"),
				"descriptions tab is  enabled");
		...
		Assert.assertTrue(documentsTabLocator.getAttribute("class").contains("disabled"),
				"Documents info tab is enabled");
		...
		Assert.assertTrue(linkedItemsTabLocator.getAttribute("class").contains("disabled"),
				"Linked Items tab is  enabled");
		Assert.assertTrue(attributesTabLocator.getAttribute("class").contains("disabled"), "Attributes tab is enabled");
		Assert.assertTrue(customPricesTabLocator.getAttribute("class").contains("disabled"),
				"Custom prices tab is enabled");
		Assert.assertTrue(categorizationTabLocator.getAttribute("class").contains("disabled"),
				"categorization tab is enabled");
		Assert.assertTrue(customFieldsTabLocator.getAttribute("class").contains("disabled"),
				"Custom fields tab is enabled");
		Assert.assertTrue(warehouseTabLocator.getAttribute("class").contains("disabled"), "Warehouse tab is enabled");
		Assert.assertTrue(customerPartNumberTabLocator.getAttribute("class").contains("disabled"),
				"Customer Part number tab is enabled");
		Assert.assertTrue(blogLinkTabLocator.getAttribute("class").contains("disabled"),
				"Blog Link info tab is enabled");
		Assert.assertTrue(userReviewTabLocator.getAttribute("class").contains("disabled"),
				"User review info tab is enabled");
		return this;
	}
	...
	public AddNewItemPageObjects verifyNonMandatoryFields() {
		...
		for (int i = 0; i < mandatoryFieldsLocator.size(); i++) {
			...
			// Assert.assertEquals(mandatoryFieldsLocator.get(i).getText().replace("*",
			// "").trim(), allMandatoryFields[i]);
		}
	...
	public AddNewItemPageObjects verifyErrorMessage(String errorMessage[]) throws InterruptedException {
		...
		for (int i = 0; i < errorMessage.length; i++) {
			...
			Assert.assertEquals(errorMessageLocator.get(i).getText().replace("\n", "").trim(), errorMessage[i],
					"expected Error Message is: " + errorMessage[i] + "");
		}
		return this;
	}
	...
	public AddNewItemPageObjects selectAndVerifyItemStatusOptions(String[] itemStatusOptions,
			String[] valuesAssignedToEachOption) {
		...
		for (int i = 0; i < itemStatusOptions.length; i++) {
			...
			Assert.assertEquals(
					((JavascriptExecutor) getDriver())
							.executeScript("return document.getElementById('" + idOfSelect + "').value;"),
					valuesAssignedToEachOption[i]);
	...
	public AddNewItemPageObjects verifyItemSavedSuccessfulMessage(String itemsavedsuccessfulmessage) {
		...
		Assert.assertTrue(ItemSavedSuccessfulMessageLocator.getText().equalsIgnoreCase(itemsavedsuccessfulmessage),
				"Invalid  message. Getting " + ItemSavedSuccessfulMessageLocator.getText() + ".");
		return this;
