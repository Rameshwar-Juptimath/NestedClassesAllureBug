...
public class CopyOfItemPageObjects extends PageFactoryInitializer{
	...
	@FindAll(value={@FindBy(xpath="//tbody[@id='copyOfItemFormId:optionsTable:tb']/descendant::td[@class='rich-table-cell ']")})
	...
	public CopyOfItemPageObjects verifyCopyItemInputFields(String inputFieldNames) throws InterruptedException {
		...
		{
		waiting.explicitWaitVisibilityOfElement(By.xpath("//table[@class='CopyItemsTable']/descendant::td[contains(text(),'"+fields[i]+"')]"), 30);
		Assert.assertTrue(getDriver().findElement(By.xpath("//table[@class='CopyItemsTable']/descendant::td[contains(text(),'"+fields[i]+"')]")).isDisplayed(),""+fields[i]+" was not displayed");
	...
	public CopyOfItemPageObjects validateInputFields(String inputField,String testData)
	{
		...
		WebElement wb=getDriver().findElement(By.xpath("//table[@class='CopyItemsTable']/descendant::input[contains(@id,'"+inputField+"') and @type='text']"));
		
		Assert.assertEquals(wb.getAttribute("value").trim(), testData," input field "+inputField+"was not taking the enterde "+testData+"value.");
	...
	public CopyOfItemPageObjects enterSearchDataForRespectiveField(String inputField,String testData) throws Exception {
		
		switch(inputField){
		case "partNumber":
			WebElement wb1=getDriver().findElement(By.xpath("//table[@class='CopyItemsTable']/descendant::input[contains(@id,'"+inputField+"') and @type='text']"));
			wb1.clear();
			wb1.sendKeys(testData);
			break;
		case "noOfCopies" :
			WebElement wb2=getDriver().findElement(By.xpath("//table[@class='CopyItemsTable']/descendant::input[contains(@id,'"+inputField+"') and @type='text']"));
			wb2.clear();
	...
	public CopyOfItemPageObjects verifyErrorMessage(String expErrorMessage) {
		Assert.assertEquals(getDriver().findElement(By.xpath("//span[contains(text(),'"+expErrorMessage+"')]")).getText().trim(), expErrorMessage.trim());
		return this;
	}
	...
	public CopyOfItemPageObjects verifyResetFunctionality(String fieldName, String testData) {
		...
		WebElement wb=getDriver().findElement(By.xpath("//table[@class='CopyItemsTable']/descendant::input[contains(@id,'"+fieldName+"') and @type='text']"));	
		Assert.assertTrue(wb.getAttribute("value").isEmpty(),""+fieldName+" is not getting cleared.expected fields get cleared but Actual value was: "+wb.getAttribute("value")+"");
