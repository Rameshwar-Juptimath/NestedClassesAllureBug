...
public class ProductEditPageObjects extends PageFactoryInitializer{
	...
	public ProductEditPageObjects dragAndDropTheItemsToProduct() throws Exception {
		...
		*/
		...
		/*WebElement src = driver.findElement(By.xpath("(//div[contains(@class,'draggable')])[1]"));
		WebElement target  = driver.findElement(By.xpath("(//div[contains(@class,'dropTargetPanel')])[1]"));
		new Actions(driver).dragAndDrop(src, target).build().perform();
		Thread.sleep(6000);*/
		
		
		
		/*Actions action = new Actions(driver);  // Configure the Action
